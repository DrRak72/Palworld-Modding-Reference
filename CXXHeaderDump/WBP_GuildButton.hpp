#ifndef UE4SS_SDK_WBP_GuildButton_HPP
#define UE4SS_SDK_WBP_GuildButton_HPP

class UWBP_GuildButton_C : public UUserWidget
{
    FPointerToUberGraphFrame UberGraphFrame;                                          // 0x0278 (size: 0x8)
    class UWidgetAnimation* Anm_GrayToRed;                                            // 0x0280 (size: 0x8)
    class UWidgetAnimation* Anm_NotmalToFocus;                                        // 0x0288 (size: 0x8)
    class UImage* Base;                                                               // 0x0290 (size: 0x8)
    class UImage* Dot_0;                                                              // 0x0298 (size: 0x8)
    class UImage* Dot_1;                                                              // 0x02A0 (size: 0x8)
    class UImage* Dot_2;                                                              // 0x02A8 (size: 0x8)
    class UImage* Dot_3;                                                              // 0x02B0 (size: 0x8)
    class UImage* Frame;                                                              // 0x02B8 (size: 0x8)
    class UImage* Frame_1;                                                            // 0x02C0 (size: 0x8)
    class UImage* FrameFlare;                                                         // 0x02C8 (size: 0x8)
    class UBP_PalTextBlock_C* Text_Title;                                             // 0x02D0 (size: 0x8)
    class UWBP_PalInvisibleButton_C* WBP_PalInvisibleButton;                          // 0x02D8 (size: 0x8)
    bool IsRedButton;                                                                 // 0x02E0 (size: 0x1)
    FDataTableRowHandle MsgId;                                                        // 0x02E8 (size: 0x10)
    FWBP_GuildButton_COnClicked OnClicked;                                            // 0x02F8 (size: 0x10)
    void OnClicked();

    void SetEnableButton(bool IsEnableFlag);
    void PreConstruct(bool IsDesignTime);
    void OnInitialized();
    void BndEvt__WBP_GuildButton_WBP_PalInvisibleButton_K2Node_ComponentBoundEvent_0_CommonButtonBaseClicked__DelegateSignature(class UCommonButtonBase* Button);
    void BndEvt__WBP_GuildButton_WBP_PalInvisibleButton_K2Node_ComponentBoundEvent_1_CommonButtonBaseClicked__DelegateSignature(class UCommonButtonBase* Button);
    void BndEvt__WBP_GuildButton_WBP_PalInvisibleButton_K2Node_ComponentBoundEvent_2_CommonButtonBaseClicked__DelegateSignature(class UCommonButtonBase* Button);
    void ExecuteUbergraph_WBP_GuildButton(int32 EntryPoint);
    void OnClicked__DelegateSignature();
}; // Size: 0x308

#endif
